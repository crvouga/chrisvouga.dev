...
/** Shared palette — readable on light and dark terminals. */
...
export const cliTheme = {
  ...
  style: {
    ...
    keysHelpTip: (keys: [key: string, action: string][]) => {
      ...
      return muted(keys.map(([key, action]) => `${key} ${action}`).join(' · '));
    },
  },
};
...
export function banner(): void {
  console.log('');
  console.log(`${accent('╭')} ${label('ws')} ${muted('workstation')}`);
  console.log(
...
export function promptMessage(name: string, description?: string): string {
  if (description === undefined || description.length === 0) return name;
  return `${title(name)}\n  ${muted(description)}`;
}
